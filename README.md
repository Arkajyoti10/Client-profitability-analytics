...
The automation layer:
  ...
  2. Sends the classified data to a free LLM API (Groq, Llama 3.1) with a prompt constrained to return only two prose sections — a summary and a prioritized recommendation — while every number in the report is pulled directly from the DataFrame, not generated by the model, eliminating any risk of the LLM misreporting a figure
